...
import { MonitorSmartphone, Calendar, PenTool } from "lucide-react";
  ...
  return (
    <div className={`relative w-full max-w-[900px] mx-auto mt-12 md:mt-20 ${className}`} style={{ perspective: "2000px" }}>
      ...
      >
        ...
        <div 
          className="relative rounded-xl md:rounded-3xl overflow-hidden bg-neutral-950 border border-white/5 shadow-2xl shadow-black/50 shadow-black/50 aspect-[16/10] z-10 p-1 md:p-2"
          style={{
        ...
        {/* Floating accents */}
        <div 
           className="absolute -right-8 -top-8 md:-right-16 md:-top-12 w-24 h-24 md:w-48 md:h-48 rounded-2xl bg-gradient-to-br from-neutral-400/20 to-transparent border border-white/5 backdrop-blur-xl -z-10"
           style={{ transform: "translateZ(-80px) rotate(15deg)" }}
        />
        <div 
           className="absolute -left-4 -bottom-4 md:-left-12 md:-bottom-8 w-20 h-20 md:w-32 md:h-32 rounded-full bg-gradient-to-tr from-neutral-600/20 to-transparent border border-white/5 backdrop-blur-xl z-20"
           style={{ transform: "translateZ(50px)" }}
        />
      </motion.div>
    </div>
...
function ZoomableImage({ src, alt, onClick, className = "" }: { src: string, alt: string, onClick: (src: string, alt: string) => void, className?: string }) {
  return (
    <motion.div 
      ...
      whileTap={{ scale: 0.99 }}
      className={`relative group cursor-zoom-in overflow-hidden shadow-2xl shadow-black/50 shadow-black/50 bg-neutral-900 shadow-lg shadow-black/50 shadow-black/50 ${className || "rounded-2xl border border-white/5"}`}
...
function ModernBackground() {
  return (
    <div className="fixed inset-0 overflow-hidden pointer-events-none z-0 mix-blend-screen">
      ...
      {/* Static Glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[800px] h-[800px] rounded-full pointer-events-none blur-3xl opacity-50 bg-[radial-gradient(circle,rgba(79,70,229,0.08)_0%,rgba(147,51,234,0.05)_40%,transparent_60%)]" />
      
      {/* Diagonal light rays */}
      <div className="absolute -top-[20%] left-[-10%] w-[40%] h-[150%] bg-indigo-500/100/100/5 rotate-45 blur-[120px]" />
      <div className="absolute top-[20%] right-[-10%] w-[30%] h-[150%] bg-purple-500/100/5 -rotate-45 blur-[120px]" />
    </div>
  );
}
    ...
    <>
      ...
      <div className="case-study-shell w-full bg-neutral-950/80 backdrop-blur-xl border border-white/5 text-white/80 font-sans mt-8 pt-12 lg:pt-24 rounded-[3rem] px-4 sm:px-6 lg:px-8 xl:px-10 mb-12 shadow-[0_0_80px_-20px_rgba(0,0,0,0.05)] relative overflow-hidden">
        ...
        <div className="relative z-10">
          {/* Header */}
          <div className="flex flex-col overflow-hidden -mt-8 md:-mt-32 -mb-20 md:-mb-56">
            <ContainerScroll
              titleComponent={
                <motion.div 
                  initial="hidden" 
                ...
                >
                  <motion.div variants={itemVariants}>
                    <div className="inline-block px-4 py-1.5 rounded-full border border-indigo-600/20 bg-indigo-500/100/10 text-indigo-400 text-xs font-mono uppercase tracking-widest mb-6">Case Study</div>
                    ...
                    <p className="text-xl md:text-2xl font-light max-w-2xl mx-auto leading-tight text-white/60 mb-12">
                      An AI-powered SEO platform transforming complex traffic data into actionable, elegant dashboards.
                    </p>
                    
                    <div className="flex flex-wrap justify-center gap-8 md:gap-16 pt-8 border-t border-white/5 max-w-3xl mx-auto">
                      <div className="text-left">
                        <p className="text-xs uppercase tracking-widest text-white/50 mb-2 font-mono flex items-center gap-2">
                          <MonitorSmartphone size={14}/> Platform
                        </p>
                      ...
                      <div className="text-left">
                        ...
                        <div className="flex flex-wrap gap-3 mt-1">
                          ...
                          <span className="px-3 py-1 rounded-full bg-neutral-900/5 border border-white/10 text-sm text-white/70">Data Visualization</span>
                        </div>
                      </div>
                    </div>
                  </motion.div>
          ...
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={containerVariants} className="mb-32">
            <motion.h3 variants={itemVariants} className="text-sm font-mono text-white/60 uppercase tracking-widest mb-16 inline-block pb-2 border-b border-white/10">01 // The Genesis</motion.h3>
            
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 text-left group">
              <div className="col-span-1 md:col-span-4">
                <h4 className="text-2xl font-light text-white group-hover:text-white/60 transition-colors">A Fragmented Landscape</h4>
              </div>
          ...
          {/* Core Objectives */}
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={containerVariants} className="mb-32">
            <motion.h3 variants={itemVariants} className="text-sm font-mono text-white/60 uppercase tracking-widest mb-16 inline-block pb-2 border-b border-white/10">02 // The Journey</motion.h3>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              <motion.div variants={itemVariants} className="p-8 rounded-3xl bg-neutral-900 shadow-lg shadow-black/50 shadow-black/50 border border-white/5 hover:bg-neutral-800/50 hover:border-indigo-500/20 hover:shadow-xl shadow-black/50 shadow-black/50 transition-all duration-300">
                 <div className="w-12 h-12 rounded-xl bg-indigo-500/100/10 text-indigo-400 flex items-center justify-center mb-6 text-xl">01</div>
                 <h4 className="text-xl font-medium text-white mb-4">Taming the Data Beast</h4>
                 <p className="text-white/60 font-light leading-relaxed">Our first major hurdle was data density. We engaged with dozens of agency owners to understand their daily workflows. The clear mandate: turn heavy analytical metrics into digestible and visually appealing charts, helping marketers find insights instantly without feeling overwhelmed by numbers.</p>
              </motion.div>
              <motion.div variants={itemVariants} className="p-8 rounded-3xl bg-neutral-900 shadow-lg shadow-black/50 shadow-black/50 border border-white/5 hover:bg-neutral-800/50 hover:border-indigo-500/20 hover:shadow-xl shadow-black/50 shadow-black/50 transition-all duration-300">
                 <div className="w-12 h-12 rounded-xl bg-indigo-500/100/10 text-indigo-400 flex items-center justify-center mb-6 text-xl">02</div>
                 <h4 className="text-xl font-medium text-white mb-4">The Mobile Imperative</h4>
                 <p className="text-white/60 font-light leading-relaxed">Most SEO tools fall apart on smaller screens. We challenged ourselves to build deeper. We ensured all complex data tables and intricate line charts remained fully functional, beautiful, and intuitive across the entire Apple ecosystem—from wide desktops down to the iPad and iPhone.</p>
              </motion.div>
              <motion.div variants={itemVariants} className="p-8 rounded-3xl bg-neutral-900 shadow-lg shadow-black/50 shadow-black/50 border border-white/5 hover:bg-neutral-800/50 hover:border-indigo-500/20 hover:shadow-xl shadow-black/50 shadow-black/50 transition-all duration-300">
                 <div className="w-12 h-12 rounded-xl bg-indigo-500/100/10 text-indigo-400 flex items-center justify-center mb-6 text-xl">03</div>
                 <h4 className="text-xl font-medium text-white mb-4">A Unified State of Mind</h4>
                 <p className="text-white/60 font-light leading-relaxed">Ultimately, it was about tool consolidation. We seamlessly combined domain overviews, organic research, and site audits natively. The result is a fluid experience where marketers never have to context-switch between applications, maintaining their strategic flow state.</p>
              </motion.div>
            </div>
          </motion.div>
          ...
          {/* Competitor Analysis */}
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={containerVariants} className="mb-32">
            <motion.h3 variants={itemVariants} className="text-sm font-mono text-white/60 uppercase tracking-widest mb-16 inline-block pb-2 border-b border-white/10">03 // Competitor Analysis</motion.h3>
            
            <motion.div variants={itemVariants} className="space-y-8 bg-neutral-900 shadow-xl shadow-black/50 shadow-black/50 p-8 md:p-12 lg:p-16 rounded-[3rem] border border-white/5 relative overflow-hidden">
              <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-neutral-900/5 rounded-full blur-[100px] pointer-events-none" />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                <div className="p-8 rounded-3xl bg-neutral-800/50 border border-white/5">
                  <div className="flex items-center gap-4 mb-4">
                    <span className="w-10 h-10 rounded-full bg-red-500/100/10 text-red-400 flex items-center justify-center text-xl">✕</span>
                    <h5 className="text-xl font-medium text-white">Legacy Enterprise Tools</h5>
                  </div>
                  <p className="text-white/60 font-light mb-6">Ahrefs, Semrush, Moz</p>
                  <ul className="space-y-3">
                    <li className="flex items-start gap-3 text-white/50 text-sm"><span className="text-red-500/70 mt-0.5">•</span> Extremely data-rich but visually cluttered.</li>
                    <li className="flex items-start gap-3 text-white/50 text-sm"><span className="text-red-500/70 mt-0.5">•</span> Steep learning curves for new marketers.</li>
                    <li className="flex items-start gap-3 text-white/50 text-sm"><span className="text-red-500/70 mt-0.5">•</span> Dated interfaces that resist modern workflows.</li>
                  </ul>
                </div>
                ...
                <div className="p-8 rounded-3xl bg-neutral-800/50 border border-white/5">
                  ...
                  <p className="text-white/60 font-light mb-6">Ubersuggest, Mangools</p>
                  <ul className="space-y-3">
                    <li className="flex items-start gap-3 text-white/50 text-sm"><span className="text-red-500/70 mt-0.5">•</span> Great UX but lack technical depth.</li>
                    <li className="flex items-start gap-3 text-white/50 text-sm"><span className="text-red-500/70 mt-0.5">•</span> Insufficient for serious enterprise audits.</li>
                    <li className="flex items-start gap-3 text-white/50 text-sm"><span className="text-red-500/70 mt-0.5">•</span> Often require buying multiple disjointed tools.</li>
                  </ul>
                </div>
              </div>
              
              <div className="p-8 mt-8 rounded-2xl bg-neutral-900/5 border border-white/20 relative z-10 flex flex-col md:flex-row gap-6 items-center">
                <div className="w-16 h-16 shrink-0 rounded-full bg-neutral-900/10 flex items-center justify-center text-white text-2xl">✓</div>
                <div>
          ...
          {/* User Flow */}
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={containerVariants} className="mb-32">
            <motion.h3 variants={itemVariants} className="text-sm font-mono text-white/60 uppercase tracking-widest mb-16 inline-block pb-2 border-b border-white/10">04 // User Flow</motion.h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 group">
               ...
               <div className="lg:col-span-7">
                  <div className="relative border-l border-white/5 ml-6 md:ml-12 space-y-20 pb-8 pt-4">
                    ...
                    <motion.div variants={itemVariants} className="relative pl-8 md:pl-16">
                      <div className="absolute w-8 h-8 rounded-full bg-indigo-600 border-2 border-indigo-600 text-white left-[-17px] top-0 flex items-center justify-center text-xs font-mono text-white">3</div>
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div className="p-4 rounded-xl bg-neutral-900 shadow-md border border-white/5 hover:border-white/20 transition-colors">
                          <h6 className="text-white font-medium text-sm mb-2">Error Logs</h6>
                          <p className="text-white/50 text-xs">Categorized list of broken links, missing tags, and slow pages.</p>
                        </div>
                        <div className="p-4 rounded-xl bg-neutral-900 shadow-md border border-white/5 hover:border-white/20 transition-colors">
                          <h6 className="text-white font-medium text-sm mb-2">Remediation Steps</h6>
                          <p className="text-white/50 text-xs">Clear, step-by-step developer instructions to fix surfaced issues.</p>
                        </div>
                      </div>
                    </motion.div>
                  </div>
          ...
          <div className="mb-32">
            <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={containerVariants} className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
              <div>
                <motion.h3 variants={itemVariants} className="text-sm font-mono text-white/60 uppercase tracking-widest inline-block pb-2 border-b border-white/10">05 // Visual Execution</motion.h3>
            ...
            <div className="space-y-32">
                ...
                <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "-50px" }} variants={containerVariants}>
                   <div className="flex items-center gap-4 mb-12">
                     <div className="h-[1px] bg-neutral-900/10 flex-grow" />
                     <h4 className="text-xl font-medium text-white/80 px-4 font-mono">Responsive Ecosystem</h4>
                     <div className="h-[1px] bg-neutral-900/10 flex-grow" />
                   </div>
                   <p className="text-white/60 text-center max-w-2xl mx-auto mb-12">Adapting intense data visualizations natively for iPad and iPhone ensures marketers can monitor critical metrics on the go.</p>
